...
[xml]$versionProps = Get-Content (Join-Path $root 'eng/Versions.props')
$sdkVersion = [string]$versionProps.Project.PropertyGroup.SdkVersion
if ([string]::IsNullOrWhiteSpace($sdkVersion)) { throw 'eng/Versions.props does not define SdkVersion.' }
...
$package = Join-Path $feed "Typst.Renderer.Native.$Rid.$sdkVersion.nupkg"
if (-not (Test-Path $package)) { throw "Missing package $package" }
$unoPackage = Join-Path $feed "Typst.Renderer.Uno.$sdkVersion.nupkg"
...
try {
    ...
    $nativeNuspecEntry = $zip.Entries | Where-Object { $_.FullName -like '*.nuspec' } | Select-Object -First 1
    if ($null -eq $nativeNuspecEntry) { throw 'Native package does not contain its nuspec.' }
    $reader = [System.IO.StreamReader]::new($nativeNuspecEntry.Open())
    try { [xml]$nativeNuspec = $reader.ReadToEnd() } finally { $reader.Dispose() }
    $nativePackageVersion = $nativeNuspec.SelectSingleNode("//*[local-name()='metadata']/*[local-name()='version']")
    if ($null -eq $nativePackageVersion -or $nativePackageVersion.InnerText -ne $sdkVersion) {
        throw "Native package version must be $sdkVersion."
    }
    $nativeCoreDependency = $nativeNuspec.SelectSingleNode(
        "//*[local-name()='dependency'][@id='Typst.Renderer']")
    if ($null -eq $nativeCoreDependency -or $nativeCoreDependency.version -ne "[$sdkVersion]") {
        throw "Native package must depend on exactly Typst.Renderer $sdkVersion."
    }
} finally { $zip.Dispose() }
...
dotnet restore (Join-Path $unoConsumer 'UnoConsumer.csproj') --source $feed --ignore-failed-sources -p:TypstPackageVersion=$sdkVersion
if ($LASTEXITCODE -ne 0) { throw 'Uno clean consumer restore failed.' }
dotnet run --project (Join-Path $unoConsumer 'UnoConsumer.csproj') -c Release --no-restore -p:TypstPackageVersion=$sdkVersion
if ($LASTEXITCODE -ne 0) { throw 'Uno clean consumer run failed.' }

if ($Rid -eq 'win-x64') {
    $winFormsPackage = Join-Path $feed "Typst.Renderer.WinForms.$sdkVersion.nupkg"
    ...
    if ($null -eq $coreDependency -or $coreDependency.version -ne $sdkVersion) {
        throw "WinForms package must depend on Typst.Renderer.Core $sdkVersion."
    }
    ...
    $wpfPackage = Join-Path $feed "Typst.Renderer.Wpf.$sdkVersion.nupkg"
    ...
    $winUiPackage = Join-Path $feed "Typst.Renderer.WinUI.$sdkVersion.nupkg"
    ...
    [xml]$winUiMetadata = $winUiNuspec
    $winUiCoreDependency = $winUiMetadata.SelectSingleNode(
        "//*[local-name()='dependency'][@id='Typst.Renderer.Core']")
    if ($null -eq $winUiCoreDependency -or $winUiCoreDependency.version -ne $sdkVersion) {
        throw "WinUI package does not depend on Typst.Renderer.Core $sdkVersion."
    }
    $windowsAppSdkDependency = $winUiMetadata.SelectSingleNode(
        "//*[local-name()='dependency'][@id='Microsoft.WindowsAppSDK']")
    if ($null -eq $windowsAppSdkDependency -or $windowsAppSdkDependency.version -ne '2.3.1') {
    ...
    dotnet restore (Join-Path $winUiConsumer 'CleanWinUiConsumer.csproj') --force --no-cache --packages $winUiPackages -p:TypstPackageVersion=$sdkVersion
    if ($LASTEXITCODE -ne 0) { throw 'WinUI clean consumer restore failed.' }
    dotnet build (Join-Path $winUiConsumer 'CleanWinUiConsumer.csproj') -c Release --no-restore -p:TypstPackageVersion=$sdkVersion
...
if ($currentRid -eq $Rid) {
    ...
    dotnet restore (Join-Path $consumer 'CleanConsumer.csproj') --ignore-failed-sources --packages $consumerPackages -p:TypstNativePackage=$packageId -p:TypstPackageVersion=$sdkVersion
    if ($LASTEXITCODE -ne 0) { throw "$Rid clean consumer restore failed." }
    dotnet run --project (Join-Path $consumer 'CleanConsumer.csproj') -c Release --no-restore -p:TypstNativePackage=$packageId -p:TypstPackageVersion=$sdkVersion
    ...
    if ($Rid -eq 'win-x64') {
        ...
        dotnet restore (Join-Path $winFormsConsumer 'WinFormsConsumer.csproj') --source $feed --ignore-failed-sources -p:TypstNativePackage=$packageId -p:TypstPackageVersion=$sdkVersion
        if ($LASTEXITCODE -ne 0) { throw "$Rid WinForms consumer restore failed." }
        dotnet run --project (Join-Path $winFormsConsumer 'WinFormsConsumer.csproj') -c Release --no-restore -p:TypstNativePackage=$packageId -p:TypstPackageVersion=$sdkVersion
        ...
        dotnet restore (Join-Path $wpfConsumer 'CleanWpfConsumer.csproj') --source $feed --ignore-failed-sources -p:TypstNativePackage=$packageId -p:TypstPackageVersion=$sdkVersion
        if ($LASTEXITCODE -ne 0) { throw 'WPF clean consumer restore failed.' }
        dotnet run --project (Join-Path $wpfConsumer 'CleanWpfConsumer.csproj') -c Release --no-restore -p:TypstNativePackage=$packageId -p:TypstPackageVersion=$sdkVersion
