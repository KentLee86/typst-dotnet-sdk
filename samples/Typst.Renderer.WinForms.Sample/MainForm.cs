...
namespace Typst.Renderer.WinForms.Sample;

public sealed class MainForm : Form
{
    private static readonly Size InitialLogicalWindowSize = new(1280, 820);
    private readonly TypstRendererOptions _rendererOptions;
    private readonly TypstViewportInteractionController _viewportInteraction;
    private TypstRenderController? _renderController;
    private readonly global::Typst.Renderer.WinForms.TypstView _preview = new()
    {
        Dock = DockStyle.Fill,
    ...
    private readonly ComboBox _demoPicker = new()
    {
        ...
        DisplayMember = nameof(TypstDemo.DisplayName)
    };
    ...
    public MainForm()
    {
        ...
        _zoomModePicker.Items.AddRange(Enum.GetValues<TypstZoomMode>().Cast<object>().ToArray());
        _viewModePicker.Items.AddRange(Enum.GetValues<TypstPageViewMode>().Cast<object>().ToArray());
        _qualityPicker.Items.AddRange(RasterQualityChoices.Cast<object>().ToArray());
        _zoomModePicker.SelectedItem = TypstZoomMode.Custom;
        _viewModePicker.SelectedItem = TypstPageViewMode.ContinuousSingle;
        ...
        _zoomModePicker.SelectedIndexChanged += (_, _) =>
        {
            if (_zoomModePicker.SelectedItem is TypstZoomMode mode)
            {
                ...
                _zoom.Enabled = mode == TypstZoomMode.Custom;
            }
            UpdateNavigation();
        };
        _viewModePicker.SelectedIndexChanged += (_, _) =>
        {
            if (_viewModePicker.SelectedItem is TypstPageViewMode mode) _preview.SetViewMode(mode);
            UpdateNavigation();
        };
        ...
        _demoPicker.Items.AddRange(TypstDemoCatalog.All.Cast<object>().ToArray());
    ...
    {
        if (_demoPicker.SelectedItem is not TypstDemo demo || _renderController is null)
            return;
        ...
        try
        {
            var document = await _renderController.RenderProjectAsync(
                ...
                options: new TypstDocumentRenderOptions
                {
                    Ppi = TypstRasterQualityPolicy.ResolvePpi(SelectedQualityMode, _preview.Zoom)
                });
            ...
            Text = $"Typst.Renderer WinForms Sample — {summary} · Typst {document.TypstVersion}";
            ScheduleAutomaticQualityRefresh();
        }
        catch (Exception exception)
        {
            ...
            Text = $"Typst.Renderer WinForms Sample — Error: {exception.Message}";
        }
        finally
        {
    ...
    private void UpdateNavigation()
    {
        ...
        var facing = _preview.ViewMode is TypstPageViewMode.ContinuousFacing or TypstPageViewMode.FacingPages;
    ...
    private void ScheduleAutomaticQualityRefresh()
    {
        ...
        var ppi = TypstRasterQualityPolicy.ResolvePpi(SelectedQualityMode, _preview.Zoom);
        if (_preview.Document is { } document && Math.Abs(document.Ppi - ppi) < 0.5f) return;
        _qualityTimer.Stop();
        _qualityTimer.Start();
    }

    private TypstViewportOffset CurrentScrollOffset()
        => new(-_preview.AutoScrollPosition.X, -_preview.AutoScrollPosition.Y);

    private void ApplyScrollOffset(TypstViewportOffset offset)
