...
public sealed partial class MainPage : Page
{
    ...
    {
        InitializeComponent();
        _viewportInteraction = new TypstViewportInteractionController(Preview);
        ...
        DemoPicker.ItemsSource = TypstDemoCatalog.All;
    ...
    {
        if (DemoPicker.SelectedItem is not TypstDemo demo)
            return;
    ...
    private void OnZoomModeChanged(object sender, SelectionChangedEventArgs args)
    {
        ...
        {
            1 => TypstZoomMode.FitWidth,
            2 => TypstZoomMode.FitPage,
            _ => TypstZoomMode.Custom
        };
        Preview.SetZoomMode(mode);
        ZoomSlider.IsEnabled = mode == TypstZoomMode.Custom;
        UpdateViewStatus();
    }
    ...
    private void OnViewModeChanged(object sender, SelectionChangedEventArgs args)
    {
        ...
        {
            1 => TypstPageViewMode.ContinuousFacing,
            2 => TypstPageViewMode.SinglePage,
            3 => TypstPageViewMode.FacingPages,
            _ => TypstPageViewMode.ContinuousSingle
        });
        UpdateViewStatus();
    }
    ...
    {
        if (_disposed || DemoPicker.SelectedItem is not TypstDemo demo)
            return;
        ...
        try
        {
            var document = await controller.RenderProjectAsync(
                ...
                {
                    Ppi = TypstRasterQualityPolicy.ResolvePpi(SelectedQualityMode, Preview.Zoom)
                });
    ...
    private TypstRenderController EnsureRenderController()
    {
        if (_renderController is not null)
    ...
    private TypstRasterQualityMode SelectedQualityMode => QualityPicker.SelectedIndex switch
    {
        0 => TypstRasterQualityMode.Fixed,
        1 => TypstRasterQualityMode.HighResolution,
        _ => TypstRasterQualityMode.Automatic
    };
    ...
    {
        if (!_loaded || _disposed || SelectedQualityMode != TypstRasterQualityMode.Automatic) return;
        var ppi = TypstRasterQualityPolicy.ResolvePpi(SelectedQualityMode, Preview.Zoom);
