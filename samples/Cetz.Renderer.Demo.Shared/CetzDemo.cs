...
public sealed class CetzDemo
{
    ...
    public CetzProject CreateProject(string? mainSource = null)
    {
        var builder = new CetzProjectBuilder().WithMainFile(MainPath);
    ...
    private static DemoFile LoadFile(string id, string fileName)
    {
        ...
        var resourceName = $"Cetz.Renderer.Demo.Shared.Resources.{resourceDirectory}.{fileName}";
        using var stream = typeof(CetzDemo).Assembly.GetManifestResourceStream(resourceName)
