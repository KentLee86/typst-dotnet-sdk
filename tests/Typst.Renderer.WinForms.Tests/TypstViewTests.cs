...
namespace Typst.Renderer.WinForms.Tests;

public sealed class TypstViewTests
{
    private static readonly Lazy<TypstRenderedDocument> FourPageDocument = new(RenderFourPages);
    ...
    {
        using var view = new TypstView { ClientSize = new Size(600, 400), Padding = Padding.Empty };
        ITypstDocumentView contract = view;
        ...
        Assert.Equal(TypstDocumentViewController.MaximumZoom, view.Zoom);
        ...
        Assert.Equal(TypstDocumentViewController.DefaultPageSpacing, view.PageSpacing);

        view.ZoomMode = TypstZoomMode.FitWidth;
        Assert.Equal(TypstZoomMode.FitWidth, view.ZoomMode);
        Assert.Equal(600, view.Layout.ExtentWidth, precision: 2);

        contract.SetZoomMode(TypstZoomMode.FitPage);
        Assert.Equal(TypstZoomMode.FitPage, view.ZoomMode);
        view.ViewMode = TypstPageViewMode.FacingPages;
    ...
    public void NavigationLayoutAndReleaseUseCommonBehaviorAndClearResources()
    {
        ...
        view.SetViewMode(TypstPageViewMode.SinglePage);
    ...
    public void ContinuousNavigationMovesTheWinFormsScrollHostToTheSelectedPage()
    {
        using var view = new TypstView
        {
            ...
            ViewMode = TypstPageViewMode.ContinuousSingle
        };
        view.CreateControl();
    ...
    public void PremultipliedRgbaConversionUsesGdiBgraChannelOrder()
    {
        ...
        TypstBitmapConverter.ConvertPremultipliedRgbaToBgra(rgba, bgra);
    ...
    private static TypstRenderedDocument RenderFourPages()
    {
    ...
    private static string NativePath()
    {
        ...
        return Path.Combine(root, "artifacts", "native", "win-x64", "typst_dotnet_native.dll");
    }
}
