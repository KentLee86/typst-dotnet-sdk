...
public sealed class TypstViewportTests
{
    [AvaloniaFact]
    public async Task RepeatedCtrlWheelZoomKeepsTheSamePagePointUnderThePointer()
    {
        ...
        using var viewport = new TypstViewport();
    ...
    public async Task AutomaticQualityRerenderIncreasesBackingPixelsWithoutChangingZoom()
    {
        ...
        try
        {
            ...
            var targetPpi = TypstRasterQualityPolicy.ResolvePpi(TypstRasterQualityMode.Automatic, zoom);
    ...
    private static Window Show(TypstViewport viewport)
    {
    ...
    private static Task<TypstRenderedDocument> RenderDocument(TypstDocumentRenderer renderer, float ppi = 96) =>
    ...
    private static Task<TypstRenderedDocument> RenderLongDocument(TypstDocumentRenderer renderer) =>
        renderer.RenderSourceAsync(
            "#set page(width: 400pt, height: 500pt, margin: 20pt)\n#set text(size: 36pt)\n" +
