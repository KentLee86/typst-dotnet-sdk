...
internal sealed class NativeApi : IDisposable
{
    ...
    internal static NativeApi Load(string? overridePath)
    {
        ...
        {
            library = NativeLibrary.Load("typst_dotnet_native", Assembly.GetExecutingAssembly(),
    ...
    internal void ThrowIfError(int status, IntPtr errorPointer)
    {
        ...
        throw kind switch
        {
            "configuration" => new TypstConfigurationException(message),
            "compilation" => new TypstCompilationException(message, MapDiagnostics(error?.Diagnostics)),
            _ => new TypstRenderException(message, kind)
        };
    }
    ...
    private static IReadOnlyList<TypstDiagnostic> MapDiagnostics(IEnumerable<NativeDiagnostic>? diagnostics)
        => diagnostics?.Select(item => new TypstDiagnostic(
