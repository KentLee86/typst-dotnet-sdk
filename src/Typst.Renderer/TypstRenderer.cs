...
public sealed class TypstRenderer : IDisposable
{
    ...
    {
        options ??= new TypstRendererOptions();
        ...
        try
        {
            ...
            if (_native.AbiVersion() != 1)
                throw new TypstNativeLibraryException($"Native ABI {_native.AbiVersion()} is incompatible; expected 1.");
            _handle = CreateRenderer(options);
        }
        catch (TypstRenderException) { throw; }
        ...
        {
            throw new TypstNativeLibraryException("Unable to load the CeTZ native runtime for this process architecture.", exception);
        }
    }

    public TypstRenderResult RenderFile(string path, TypstRenderSettings? settings = null)
    {
    ...
    public Task<TypstRenderResult> RenderFileAsync(string path, TypstRenderSettings? settings = null, CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(path);
        var absolute = Path.GetFullPath(path);
        ValidateFileAccess(absolute);
    ...
    public TypstRenderResult RenderSource(string source, string virtualPath = "main.typ", TypstRenderSettings? settings = null)
    {
        ArgumentNullException.ThrowIfNull(source);
        var project = new TypstProjectBuilder().WithMainFile(virtualPath).AddText(virtualPath, source).Build();
        return RenderProject(project, settings);
    }

    public Task<TypstRenderResult> RenderSourceAsync(string source, string virtualPath = "main.typ", TypstRenderSettings? settings = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(source);
        var project = new TypstProjectBuilder().WithMainFile(virtualPath).AddText(virtualPath, source).Build();
    ...
    public TypstRenderResult RenderProject(TypstProject project, TypstRenderSettings? settings = null)
    {
        ...
        {
            ThrowIfDisposed();
            return RenderProjectCore(project, settings ?? new TypstRenderSettings());
        }
        finally { _gate.Release(); }
    }

    public async Task<TypstRenderResult> RenderProjectAsync(TypstProject project, TypstRenderSettings? settings = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(project);
        ThrowIfDisposed();
        ...
        try
        {
            ...
            return await Task.Run(() => RenderProjectCore(project, settings ?? new TypstRenderSettings()), CancellationToken.None).ConfigureAwait(false);
        }
        finally { _gate.Release(); }
    }

    private RendererSafeHandle CreateRenderer(TypstRendererOptions options)
    {
        ...
        if (!Directory.Exists(baseDirectory)) throw new TypstConfigurationException($"BaseDirectory does not exist: {baseDirectory}");
        var config = JsonSerializer.Serialize(new
        {
            ...
            {
                TypstPackageResolution.CacheThenNetwork => "cache_then_network",
                TypstPackageResolution.CacheOnly => "cache_only",
                _ => "embedded_only"
            },
    ...
    private TypstRenderResult RenderProjectCore(TypstProject project, TypstRenderSettings settings)
    {
        ...
        var metadata = JsonSerializer.Deserialize<NativeResponse>(json, NativeApi.JsonOptions)
            ?? throw new TypstRenderException("Native runtime returned empty result metadata.", "protocol");
        var count = checked((int)_native.ResultCount(result.DangerousGetHandle()));
        if (count != metadata.Outputs.Count)
            throw new TypstRenderException("Native artifact count does not match metadata.", "protocol");
        var artifacts = new List<TypstArtifact>(count);
        for (var index = 0; index < count; index++)
        {
            ...
            if (length > int.MaxValue) throw new TypstRenderException("Artifact is too large for a managed byte array.", "protocol");
            var bytes = new byte[(int)length];
            if (bytes.Length > 0) Marshal.Copy(data, bytes, 0, bytes.Length);
            var item = metadata.Outputs[index];
            if (item.Bytes != bytes.Length) throw new TypstRenderException("Native artifact byte count does not match metadata.", "protocol");
            artifacts.Add(new TypstArtifact(ParseFormat(item.Format), item.Page, item.FileName, bytes,
                item.Width, item.Height, item.Stride,
                item.PixelFormat == "rgba8_premultiplied" ? TypstPixelFormat.Rgba8Premultiplied : TypstPixelFormat.None));
        }
        var diagnostics = metadata.Diagnostics.Select(item => new TypstDiagnostic(
            item.Severity == "error" ? TypstDiagnosticSeverity.Error : TypstDiagnosticSeverity.Warning,
            item.Message, item.File, item.Line, item.Column, item.Hints)).ToArray();
        return new TypstRenderResult(metadata.TypstVersion, artifacts, diagnostics,
            new TypstRenderTiming(metadata.EngineInitMs, metadata.CompileMs, metadata.ExportMs, metadata.TotalMs));
    }

    private ProjectSafeHandle CreateProject(TypstProject project)
    {
    ...
    private static string FormatName(TypstOutputFormat format) => format switch
    {
        TypstOutputFormat.Svg => "svg", TypstOutputFormat.Png => "png",
        TypstOutputFormat.Pdf => "pdf", TypstOutputFormat.Rgba => "rgba",
    ...
    private void ValidateFileAccess(string absolutePath)
    {
        ...
        if (relative == ".." || relative.StartsWith(".." + Path.DirectorySeparatorChar, StringComparison.Ordinal) || Path.IsPathRooted(relative))
            throw new TypstConfigurationException($"Input '{absolutePath}' is outside restricted directory '{_restrictedDirectory}'.");
    }

    private static TypstOutputFormat ParseFormat(string format) => format switch
    {
        "svg" => TypstOutputFormat.Svg, "png" => TypstOutputFormat.Png,
        "pdf" => TypstOutputFormat.Pdf, "rgba" => TypstOutputFormat.Rgba,
        _ => throw new TypstRenderException($"Unknown native artifact format: {format}", "protocol")
    };
