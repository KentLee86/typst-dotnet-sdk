...
namespace Typst.Renderer.Avalonia;

/// <summary>Displays every page of a UI-neutral CeTZ rendered document.</summary>
public sealed class TypstView : Control, ITypstDocumentView, IDisposable
{
    public static readonly StyledProperty<TypstRenderedDocument?> DocumentProperty =
        AvaloniaProperty.Register<TypstView, TypstRenderedDocument?>(nameof(Document));

    public static readonly StyledProperty<double> ZoomProperty =
        AvaloniaProperty.Register<TypstView, double>(nameof(Zoom), 1d, coerce: CoerceZoom);

    public static readonly StyledProperty<double> PageSpacingProperty =
        AvaloniaProperty.Register<TypstView, double>(nameof(PageSpacing), TypstDocumentViewController.DefaultPageSpacing,
            coerce: (_, value) => TypstDocumentViewController.NormalizePageSpacing(value));

    public static readonly StyledProperty<TypstZoomMode> ZoomModeProperty =
        AvaloniaProperty.Register<TypstView, TypstZoomMode>(nameof(ZoomMode));

    public static readonly StyledProperty<TypstPageViewMode> ViewModeProperty =
        AvaloniaProperty.Register<TypstView, TypstPageViewMode>(nameof(ViewMode));
    ...
    static TypstView()
    {
        AffectsMeasure<TypstView>(DocumentProperty, ZoomProperty, PageSpacingProperty, ZoomModeProperty, ViewModeProperty);
        AffectsRender<TypstView>(DocumentProperty, ZoomProperty, PageSpacingProperty, ZoomModeProperty, ViewModeProperty);
    }
    ...
    public void SetDocument(TypstRenderedDocument document) => Document = document;
    public void SetZoom(double zoom) => Zoom = zoom;
    public void SetZoomMode(TypstZoomMode mode) => ZoomMode = mode;
    public void SetViewMode(TypstPageViewMode mode) => ViewMode = mode;
    ...
    protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
    {
        ...
        {
            var document = change.GetNewValue<TypstRenderedDocument?>();
        ...
        {
            _controller.SetZoomMode(change.GetNewValue<TypstZoomMode>());
            SynchronizeProperties();
        }
        else if (change.Property == ViewModeProperty)
            _controller.SetViewMode(change.GetNewValue<TypstPageViewMode>());
    }
    ...
    private static double CoerceZoom(AvaloniaObject owner, double value)
        => TypstDocumentViewController.NormalizeZoom(value);
