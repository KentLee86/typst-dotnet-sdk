...
/// </summary>
public sealed class TypstDocumentViewController
{
    ...
    private TypstRenderedDocument? _document;
    ...
    private TypstZoomMode _zoomMode;
    private TypstPageViewMode _viewMode = TypstPageViewMode.ContinuousSingle;
    private TypstDocumentViewLayout _layout = TypstDocumentViewLayout.Empty;

    public event EventHandler? Changed;

    public TypstRenderedDocument? Document => _document;
    public double Zoom => _zoom;
    public TypstZoomMode ZoomMode => _zoomMode;
    public TypstPageViewMode ViewMode => _viewMode;
    public double PageSpacing => _pageSpacing;
    public int CurrentPageIndex => _currentPageIndex;
    public int PageCount => _document?.Pages.Count ?? 0;
    public TypstDocumentViewLayout Layout => _layout;
    ...
    public void SetZoom(double zoom)
    {
        ...
        if (_zoomMode == TypstZoomMode.Custom && _customZoom.Equals(normalized)) return;
        _customZoom = normalized;
        _zoomMode = TypstZoomMode.Custom;
        RebuildLayout();
    }
    ...
    {
        if (_viewMode is TypstPageViewMode.SinglePage or TypstPageViewMode.FacingPages)
            return false;
        var normalized = SpreadStart(ClampPage(pageIndex));
    ...
    private bool IsFacing => _viewMode is TypstPageViewMode.ContinuousFacing or TypstPageViewMode.FacingPages;
    ...
    {
        _zoom = ResolveZoom();
        _layout = TypstDocumentViewLayout.Create(_document, _zoom, _pageSpacing, _viewMode, _currentPageIndex);
        Changed?.Invoke(this, EventArgs.Empty);
    }

    private double ResolveZoom()
    {
        if (_zoomMode == TypstZoomMode.Custom || _document is null || _viewportWidth <= 0 || _viewportHeight <= 0)
            return _customZoom;

        var target = TypstDocumentViewLayout.MeasureFocus(_document, _pageSpacing, _viewMode, _currentPageIndex);
        ...
        var value = _zoomMode == TypstZoomMode.FitWidth
            ? widthZoom
...
public sealed class TypstDocumentViewLayout
{
    internal static TypstDocumentViewLayout Empty { get; } = new([], 0, 0, 0, 0);

    private TypstDocumentViewLayout(IReadOnlyList<TypstPageViewLayout> pages, double logicalWidth,
        double logicalHeight, double extentWidth, double extentHeight)
        => (Pages, LogicalWidth, LogicalHeight, ExtentWidth, ExtentHeight) =
            (pages, logicalWidth, logicalHeight, extentWidth, extentHeight);

    public IReadOnlyList<TypstPageViewLayout> Pages { get; }
    public double LogicalWidth { get; }
    public double LogicalHeight { get; }
    public double ExtentWidth { get; }
    public double ExtentHeight { get; }

    internal static TypstDocumentViewLayout Create(TypstRenderedDocument? document, double zoom,
        double spacing, TypstPageViewMode mode, int currentPage)
    {
        if (document is null || document.Pages.Count == 0) return Empty;
        var indices = mode switch
        {
            TypstPageViewMode.SinglePage => [Math.Clamp(currentPage, 0, document.Pages.Count - 1)],
            TypstPageViewMode.FacingPages => Enumerable.Range(currentPage, Math.Min(2, document.Pages.Count - currentPage)).ToArray(),
            _ => Enumerable.Range(0, document.Pages.Count).ToArray()
        };
        var facing = mode is TypstPageViewMode.ContinuousFacing or TypstPageViewMode.FacingPages;
    ...
    internal static (double Width, double Height, double HorizontalSpacing, double VerticalSpacing) MeasureFocus(
        TypstRenderedDocument document, double spacing, TypstPageViewMode mode, int currentPage)
    {
        var index = Math.Clamp(currentPage, 0, document.Pages.Count - 1);
        var first = document.Pages[index];
        if (mode is not (TypstPageViewMode.ContinuousFacing or TypstPageViewMode.FacingPages) || index + 1 >= document.Pages.Count)
    ...
    private static TypstDocumentViewLayout CreateSingle(TypstRenderedDocument document, int[] indices, double zoom, double spacing)
    {
        ...
        var layouts = new List<TypstPageViewLayout>(indices.Length);
        var y = 0d;
    ...
    private static TypstDocumentViewLayout CreateFacing(TypstRenderedDocument document, int[] indices, double zoom, double spacing)
    {
...
public readonly record struct TypstPageViewLayout(int PageIndex, double X, double Y, double Width, double Height);
