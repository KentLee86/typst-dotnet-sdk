...
/// </summary>
public sealed class TypstRenderController : IDisposable
{
    private readonly object _sync = new();
    private readonly ITypstDocumentView _view;
    private readonly TypstDocumentRenderer _renderer;
    ...
    public TypstRenderController(
        ITypstDocumentView view,
        TypstRendererOptions? rendererOptions = null,
        SynchronizationContext? viewContext = null)
    {
        _view = view ?? throw new ArgumentNullException(nameof(view));
        _renderer = new TypstDocumentRenderer(rendererOptions);
    ...
    public Task<TypstRenderedDocument?> RenderSourceAsync(string source, string virtualPath = "main.typ",
        TypstDocumentRenderOptions? options = null, CancellationToken cancellationToken = default)
        => RenderLatestAsync(token => _renderer.RenderSourceAsync(source, virtualPath, options, token), cancellationToken);

    public Task<TypstRenderedDocument?> RenderProjectAsync(TypstProject project,
        TypstDocumentRenderOptions? options = null, CancellationToken cancellationToken = default)
        => RenderLatestAsync(token => _renderer.RenderProjectAsync(project, options, token), cancellationToken);

    public Task<TypstRenderedDocument?> RenderFileAsync(string path,
        TypstDocumentRenderOptions? options = null, CancellationToken cancellationToken = default)
    ...
    private async Task<TypstRenderedDocument?> RenderLatestAsync(
        Func<CancellationToken, Task<TypstRenderedDocument>> render,
